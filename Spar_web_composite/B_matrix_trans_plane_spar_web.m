function kg_trans_plane=B_matrix_trans_plane(gauss_points_x,gauss_points_y,sweep_angle,b,cr,I,J,K,L,M,N,P,Q,R,S,weights_x,weights_y,t_1,h_1,h,theta,thickness_of_each_ply)
...
P_non_derive_M_N_x(1,2)=(t_1/c)*gauss_points_x(j)+spar_position_M_N(1);
P_derive_M_N_x(1,1)=0;
P_derive_M_N_x(1,2)=(t_1/c);
...
P_non_derive_M_N_y(1,2)=(t_1/c)*gauss_points_y(op)+spar_position_M_N(1);
P_derive_M_N_y(1,1)=0;
P_derive_M_N_y(1,2)=(t_1/c);
...
P_derive_R_S_y(1,2)=1;
        ...
        for k=3:number_of_nodes_M_N
           P_derive_M_N_x(1,k)=((((2*k+1)/(k+1))*(t_1/c)*gauss_points_x(j)+spar_position_M_N(1,k-1))*P_derive_M_N_x(1,k-1)-(k/(k+1)*P_derive_M_N_x(1,k-2))); 
           P_non_derive_M_N_x(1,k)=((((2*k+1)/(k+1))*(t_1/c)*gauss_points_x(j)+spar_position_M_N(1,k-1))*P_non_derive_M_N_x(1,k-1)-(k/(k+1)*P_non_derive_M_N_x(1,k-2))); 
        end
        ...
        for k=3:number_of_nodes_M_N
           P_derive_M_N_y(1,k)=((((2*k+1)/(k+1))*(t_1/c)*gauss_points_y(op)+spar_position_M_N(1,k-1))*P_derive_M_N_y(1,k-1)-(k/(k+1)*P_derive_M_N_y(1,k-2))); 
           P_non_derive_M_N_y(1,k)=((((2*k+1)/(k+1))*(t_1/c)*gauss_points_y(op)+spar_position_M_N(1,k-1))*P_non_derive_M_N_y(1,k-1)-(k/(k+1)*P_non_derive_M_N_y(1,k-2))); 
        end
        for k=3:number_of_nodes_P_Q
           P_derive_P_Q_y(1,k)=((((2*k+1)/(k+1))*(t_1/c)*gauss_points_y(op)+spar_position_P_Q(1,k-1))*P_derive_P_Q_y(1,k-1)-(k/(k+1)*P_derive_P_Q_y(1,k-2))); 
           P_non_derive_P_Q_y(1,k)=((((2*k+1)/(k+1))*(t_1/c)*gauss_points_y(op)+spar_position_P_Q(1,k-1))*P_non_derive_P_Q_y(1,k-1)-(k/(k+1)*P_non_derive_P_Q_y(1,k-2))); 
        end
        for k=3:number_of_nodes_R_S
           P_derive_R_S_y(1,k)=((((2*k+1)/(k+1))*(t_1/c)*gauss_points_y(op)+spar_position_R_S(1,k-1))*P_derive_R_S_y(1,k-1)-(k/(k+1)*P_derive_R_S_y(1,k-2))); 
           P_non_derive_R_S_y(1,k)=((((2*k+1)/(k+1))*(t_1/c)*gauss_points_y(op)+spar_position_R_S(1,k-1))*P_non_derive_R_S_y(1,k-1)-(k/(k+1)*P_non_derive_R_S_y(1,k-2))); 
        end
...
%         final_RS=[final_RS_x;final_RS_y];
        ...
        D=D_matrix_general_trans_plane(sweep_angle,cr,b,theta,thickness_of_each_ply,gauss_points_x,gauss_points_y,coeff_of_z_lower,h_1,coeff_of_z_upper,h);%+D_matrix_upper_airfoil_trans_plane(sweep_angle,cr,b,theta,thickness_of_each_ply,gauss_points_x,gauss_points_y,coeff_of_z_upper,h_1,h);
        hope=hope+(t_1/c)*B'*D*B*weights_x(j)*weights_y(op)*detJacobian;
    end
    end
