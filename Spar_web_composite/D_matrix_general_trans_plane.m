function D_matrix_trans_1=D_matrix_general_trans_plane(sweep,cr,b,theta,thickness_of_each_ply,gauss_points_x,gauss_points_y,coeff_of_z_lower,h_1,coeff_of_z_upper,h)
...
x=z_lower_l(1:end,1);
y_1=z_upper_curve(1:end,1);
%z_lower_1=z_lower(1:end,1);
for po=1:length(coeff_of_z_lower)
    z_derive_l_lower=z_derive_l_lower+(coeff_of_z_lower(po)*(((factorial(length(coeff_of_z_lower))/(factorial(length(coeff_of_z_lower)-po)*factorial(po))))*po*(length(coeff_of_z_lower)-po)*((z_lower_l(po))^(po-1)))*((z_lower_l(po))^0.75)*((1-z_lower_l(po))^0.75));
    z_derive_2_lower=z_derive_2_lower+(coeff_of_z_lower(po))*(factorial(length(coeff_of_z_lower))/((factorial(length(coeff_of_z_lower)-po)*factorial(po))))*(0.75)*(z_lower_l(po))^(0.75-1)*(1-z_lower_l(po))^0.75;
     z_derive_3_lower=z_derive_3_lower+(coeff_of_z_lower(po)*(((factorial(length(coeff_of_z_lower))/(factorial(length(coeff_of_z_lower))*(0.75*(1-z_lower_l(po))^(0.75-1)*(z_lower_l(po))^(0.75)))))));
end
for po=1:length(coeff_of_z_upper)
    z_derive_l_upper=z_derive_l_upper+(coeff_of_z_upper(po)*(((factorial(length(coeff_of_z_upper))/(factorial(length(coeff_of_z_upper)-po)*factorial(po))))*po*(length(coeff_of_z_upper)-po)*((z_upper_curve(po))^(po-1)))*((z_upper_curve(po))^0.75)*((1-z_upper_curve(po))^0.75));
...
z_derive_lower=z_derive_l_lower+z_derive_2_lower+z_derive_3_lower;
y=atan(z_derive_lower);
y=(y*180/pi);
thickness_lower_1=thickness_lower*sqrt(1+((tan(y)))^2);
z_derive_upper=z_derive_l_upper+z_derive_2_upper+z_derive_3_upper;
...
for k=1:length(y_1)
z_2_upper=z_2_upper+(factorial(length(coeff_of_z_upper))/(factorial(length(coeff_of_z_upper)-k)*factorial(k)))*coeff_of_z_upper*(((y_1).^k).*(ones(1,length(coeff_of_z_upper))-y_1).^(length(coeff_of_z_upper)-k))';
end
z_1=z_1';
...
for i=1:length(n)
    ...
    integral_final=integral_final*3*h_1(1)/8;
end
