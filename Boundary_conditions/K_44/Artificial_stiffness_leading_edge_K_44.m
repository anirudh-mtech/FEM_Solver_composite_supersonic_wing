function K_44_alpha_le_trans=Artificial_stiffness_leading_edge(I,J,K,L,M,N,P,Q,R,S,sweep_angle,span_of_the_entire_wing,root_of_chord,gauss_points_x,gauss_points_y,weights_x,weights_y)
%  I=2;
% J=2;
% K=2;
%  L=2;
...
x_3=coordinates_x(3);
y_3=coordinates_y(3);
K_44_alpha_le_trans=zeros(1,I*J);
   ...
   for k=3:number_of_nodes_K_L_le_y
    ...
    for k=3:number_of_nodes_M_N_le_y
     ...
     final_IJ=P_non_derive_I_J_x.*P_non_derive_I_J_y;
        ...
        K_44_alpha_le_trans=K_44_alpha_le_trans+(sin(sweep_angle_in_radians)*abs(x_2-x_3)+cos(sweep_angle_in_radians)*(abs(y_2-y_3))/4)*(final_IJ_le.*final_KL_le.*final_PQ_le.*final_MN_le.*final_RS_le)'*(final_PQ)*weights_x(j)*weights_y(op);
    end
end