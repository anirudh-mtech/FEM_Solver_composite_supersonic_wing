   ...
   for k=3:number_of_nodes_K_L_te_y
    ...
    for k=3:number_of_nodes_M_N_te_y
     ...
     final_IJ=P_non_derive_I_J_x.*P_non_derive_I_J_y;
        ...
        K_33_alpha_te_trans=K_33_alpha_te_trans+(sin(0)*(x_3-x_2)+cos(0)*((y_3-y_2))/4)*(final_IJ_le.*final_KL_le.*final_PQ_le.*final_MN_le.*final_RS_le)'*(final_PQ)*weights_x(j)*weights_y(op);
    end
end
end