function K_11_alpha_root_trans=Artificial_stiffness_root(I,J,K,L,M,N,P,Q,R,S,sweep_angle,span_of_the_entire_wing,root_of_chord,gauss_points_x,gauss_points_y,weights_x,weights_y)
%  I=2;
% J=2;
% K=2;
%  L=2;
...
x_4=coordinates_x(2);
y_1=coordinates_y(1);
K_11_alpha_root_trans=zeros(1,I*J);
...
P_non_derive_R_S_x_root(1,2)=(gauss_points_x(j));
   ...
   for k=3:number_of_nodes_K_L_le_y
    ...
    for k=3:number_of_nodes_M_N_le_y
     ...
     final_IJ=P_non_derive_I_J_x.*P_non_derive_I_J_y;
        ...
        K_11_alpha_root_trans=K_11_alpha_root_trans+(abs(x_1-x_4)/4)*(final_IJ_root.*final_KL_root.*final_PQ_root.*final_MN_root.*final_RS_root)'*(final_IJ)*weights_x(j)*weights_y(op);
    end
end
end