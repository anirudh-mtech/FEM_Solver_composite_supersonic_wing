function K_11_alpha_te_trans=Artificial_stiffness_trailing_edge_K_11(I,J,K,L,M,N,P,Q,R,S,sweep_angle,span_of_the_entire_wing,root_of_chord,gauss_points_x,gauss_points_y,weights_x,weights_y)
%  I=2;
% J=2;
% K=2;
%  L=2;
...
x_2=coordinates_x(2);
y_2=coordinates_y(2);
K_11_alpha_te_trans=zeros(1,I*J);
   ...
   for k=3:number_of_nodes_K_L_te_y
    ...
    for k=3:number_of_nodes_M_N_te_y
     ...
     final_IJ=P_non_derive_I_J_x.*P_non_derive_I_J_y;
        ...
        K_11_alpha_te_trans=K_11_alpha_te_trans+(sin(0)*(x_3-x_2)+cos(0)*((y_3-y_2))/4)*(final_IJ_le.*final_KL_le.*final_PQ_le.*final_MN_le.*final_RS_le)'*(final_IJ)*weights_x(j)*weights_y(op);
    end
end