   ...
   for k=3:number_of_nodes_K_L_le_y
    ...
    for k=3:number_of_nodes_M_N_le_y
     ...
     final_IJ=P_non_derive_I_J_x.*P_non_derive_I_J_y;
        ...
        K_33_alpha_tip_trans=K_33_alpha_tip_trans+((x_3-x_4)/4)*(final_IJ_root.*final_KL_root.*final_PQ_root.*final_MN_root.*final_RS_root)'*(final_MN)*weights_x(j)*weights_y(op);
    end
end
end