function D_matrix_2=D_matrix_upper_airfoil_in_plane(sweep_angle,cr,b,theta,thickness_of_each_ply,gauss_points_x,gauss_points_y,coeff_of_z_upper,h_1)
...
integral_final=letsintegrate(z_upper,z_l_lower,sweep_angle,cr,b,gauss_points_x,gauss_points_y);
for i=1:length(n)
    k=z_l_lower+i*h;
    for lo=1:length(z_l_lower)
    if rem(k(lo),3)==0
        integral_final=integral_final+2*(function_1(k(lo),sweep_angle,cr,b,gauss_points_x,gauss_points_y));
    else
        integral_final=integral_final+3*(function_1(k(lo),sweep_angle,cr,b,gauss_points_x,gauss_points_y));
    end
    end
